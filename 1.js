...
function getCode(code) {
    return new Promise((resolve, reject) => {
        ...
        GM_xmlhttpRequest({
            method: "POST",
            url: "http://ca.zwhyzzz.top:8092/identify_GeneralCAPTCHA",
            ...
            onload: function (response) {
                if (response.status == 200) {
                    if (response.responseText && response.responseText.indexOf("触发限流策略") != -1) {
                        addText(response.response["msg"] || "触发限流策略");
                        resolve(null);
                        return;
                    }
                    try {
                        ...
                        if (response.responseText && response.responseText.indexOf("接口请求频率过高") != -1) {
                            addText("接口请求频率过高");
                        } else {
                            addText("识别结果解析失败");
                        }
                    ...
                    try {
                        if (response.response && response.response["result"] == null) {
                            addText("识别失败：返回空结果");
                        } else {
                            addText("识别失败");
                        }
            ...
            onerror: function (error) {
                addText(`请求失败: ${error.statusText || '网络错误'} ${error.status || ''}`);
                resolve(null);
            },
        });
    });
...
function matchUrl(){
    ...
    // 获取保存的URLs
    let urls = "cdcas.yuruixxkj.com|cdcas.rurenkj.com|cdcas.yeruikeji.com|"
